    ...
    const query = `
      WITH CTE AS (
        SELECT
          ...
          a.numcount as jml,
          COALESCE(a.discount,0) as diskon_1,
          COALESCE(a.amount,0)+COALESCE(a.potongan,0) as total_harga,
          CASE
            WHEN a.discount= 0 THEN COALESCE(COALESCE(COALESCE(((COALESCE(a.amount,0)+COALESCE(a.potongan,0))*pr.discount_percent)/100), pr.discount_amount*a.numcount), 0)
            WHEN a.discount= 0 AND pr1.base_id IS NOT NULL THEN COALESCE(COALESCE((((round(COALESCE(a.amount,0)+COALESCE(a.potongan,0)))*pr1.discount_percent)/100),pr1.discount_amount*a.numcount),0)
            ELSE COALESCE(a.discount,0)
          END AS diskon
        FROM bill_patient_row a
          ...
          LEFT JOIN promo_rules pr ON pr.plid=b.plid AND pr.bphid=a.bphid AND pr.base_id IS NULL
          LEFT JOIN promo_rules pr1 ON pr.plid=b.plid AND pr.base_id=a.base_id
      ...
      SELECT
        ...
        a.diskon,
        CASE
          WHEN a.diskon_1 > 0 THEN a.total_harga
          ELSE a.total_harga-a.diskon
        END::numeric(18,2) AS tot_harga
      FROM cte a
    `;
